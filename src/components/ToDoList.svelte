...
<div class="overflow-y-auto w-full h-80 max-h-80 content-end flex flex-col-reverse">
    ...
    <div class="flex flex-row justify-start h-8 items-center my-2">
        ...
        <p>{item.s}</p>
        <button class="border-dark-100 border-1 rounded-lg py-1 mx-2 text-white ml-auto" on:click={moveToDoItem(listItems.indexOf(item), false)}><i class="fa-solid fa-arrow-down"></i></button>
        <button class="border-dark-100 border-1 rounded-lg py-1 mx-2 text-white" on:click={moveToDoItem(listItems.indexOf(item), true)}><i class="fa-solid fa-arrow-up"></i></button>
        <button class="border-dark-100 border-1 rounded-lg py-1 mx-4 text-white" on:click={removeToDoItem(listItems.indexOf(item))}><i class="fa-solid fa-trash-can"></i></button>
    </div>
    {/each}
</div>
<div class="m-4">
